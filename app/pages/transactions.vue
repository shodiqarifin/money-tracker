...
async function handleSubmit() {
  ...
  if (!form.amount || isNaN(amount) || amount <= 0) {
    formError.value = 'Jumlah harus berupa angka lebih dari 0'
    return
  }
  if (!form.categoryId) {
  ...
  try {
    const payload = { amount, categoryId: form.categoryId, description: form.description, date: form.date }
...
async function confirmDelete() {
  ...
  try {
    ...
    deleteError.value = e instanceof Error ? e.message : 'Gagal hapus transaksi'
  } finally {
...
<template>
  <div class="max-w-3xl mx-auto p-4">
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <div v-if="tx.description" class="text-xs text-muted mt-0.5">{{ tx.description }}</div>
    ...
    >
      <div class="bg-surface border border-white/10 rounded-2xl p-6 w-full max-w-md mx-4">
        ...
        <div class="flex gap-2 mt-6">
          ...
          >
            {{ submitLoading ? 'Menyimpan...' : (editingTransaction ? 'Simpan' : 'Tambah') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <ConfirmModal
      ...
      @cancel="pendingDeleteId = null; deleteError = null"
    />
  </div>
</template>
